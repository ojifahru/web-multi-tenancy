<x-public.layout :tenant="$tenant" :title="__('contact.title')">
    <section class="bg-uniba-surface-1 py-20 sm:py-24">
        <x-public.container>
            <x-public.page-header :title="__('contact.title')"
                :subtitle="__('contact.subtitle')"
                :breadcrumbs="[['label' => __('menu.beranda'), 'href' => localized_route('public.home')], ['label' => __('contact.title')]]">
                <x-slot:icon>
            ...
            @if ($errors->any())
                <div class="mt-6 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                    <div class="font-semibold">{{ __('contact.form_error_heading') }}</div>
            ...
            <div class="mt-12 grid gap-6 lg:grid-cols-3">
                <x-public.card class="p-6 lg:col-span-2">
                    <div class="text-sm font-semibold text-uniba-primary-blue">{{ __('contact.form_title') }}</div>
                    <p class="mt-2 text-sm text-uniba-secondary">
                        {{ __('contact.form_description') }}
                    </p>

                    <form class="mt-5 grid gap-4 sm:grid-cols-2" action="{{ localized_route('public.contact.store') }}"
                        method="POST">
                        @csrf
                        ...
                        <div>
                            ...
                            <input id="email" name="email" type="email" value="{{ old('email') }}"
                                ...
                                class="mt-2 w-full rounded-lg border border-uniba-border bg-white px-3 py-2 text-sm text-uniba-text placeholder:text-uniba-text-secondary focus:border-uniba-primary-blue focus:outline-none focus:ring-2 focus:ring-uniba-primary-blue/20">
                            @error('email')
                                <p id="email-error" class="mt-1 text-xs font-medium text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            <label for="phone"
                                class="text-xs font-semibold uppercase tracking-[0.12em] text-uniba-primary-blue">
                                {{ __('contact.fields.phone') }}
                            </label>
                            <input id="phone" name="phone" type="text" value="{{ old('phone') }}"
                                placeholder="{{ __('contact.placeholders.phone') }}"
                                @error('phone') aria-invalid="true" aria-describedby="phone-error" @enderror
                                class="mt-2 w-full rounded-lg border border-uniba-border bg-white px-3 py-2 text-sm text-uniba-text placeholder:text-uniba-text-secondary focus:border-uniba-primary-blue focus:outline-none focus:ring-2 focus:ring-uniba-primary-blue/20">
                            @error('phone')
                                <p id="phone-error" class="mt-1 text-xs font-medium text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            <label for="subject"
                                class="text-xs font-semibold uppercase tracking-[0.12em] text-uniba-primary-blue">
                                {{ __('contact.fields.subject') }}
                            </label>
                            <input id="subject" name="subject" type="text" value="{{ old('subject') }}"
                                placeholder="{{ __('contact.placeholders.subject') }}"
                                @error('subject') aria-invalid="true" aria-describedby="subject-error" @enderror
                                class="mt-2 w-full rounded-lg border border-uniba-border bg-white px-3 py-2 text-sm text-uniba-text placeholder:text-uniba-text-secondary focus:border-uniba-primary-blue focus:outline-none focus:ring-2 focus:ring-uniba-primary-blue/20">
                            @error('subject')
                                <p id="subject-error" class="mt-1 text-xs font-medium text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="sm:col-span-2">
                            <label for="message"
                                class="text-xs font-semibold uppercase tracking-[0.12em] text-uniba-primary-blue">
                                {{ __('contact.fields.message') }}
                            </label>
                            <textarea id="message" name="message" rows="5" placeholder="{{ __('contact.placeholders.message') }}"
                            ...
                            @error('message')
                                <p id="message-error" class="mt-1 text-xs font-medium text-red-600">{{ $message }}</p>
                            @enderror
                        </div>

                        <div class="flex flex-col gap-3 sm:col-span-2 sm:flex-row sm:items-center sm:justify-between">
                            <p class="text-xs text-uniba-secondary">{{ __('contact.required_note') }}</p>
                            <x-public.button type="submit">{{ __('contact.submit_button') }}</x-public.button>
                        </div>
                    </form>
                        ...
                        <div class="mt-3 flex flex-wrap gap-3 text-sm">
                            @if (!empty($tenant->facebook_link))
                                <a class="inline-flex h-9 w-9 items-center justify-center rounded-full border border-uniba-border text-uniba-text-secondary transition hover:border-uniba-primary-blue hover:text-uniba-primary-blue"
                                    ...
                                    aria-label="Facebook">
                                    <span class="sr-only">{{ __('contact.social.facebook') }}</span>
                            ...
                            @if (!empty($tenant->instagram_link))
                                <a class="inline-flex h-9 w-9 items-center justify-center rounded-full border border-uniba-border text-uniba-text-secondary transition hover:border-uniba-primary-blue hover:text-uniba-primary-blue"
                                    ...
                                    <span class="sr-only">{{ __('contact.social.instagram') }}</span>
                                    <svg class="h-4 w-4" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                                        <path
                            ...
                            @if (!empty($tenant->twitter_link))
                                <a class="inline-flex h-9 w-9 items-center justify-center rounded-full border border-uniba-border text-uniba-text-secondary transition hover:border-uniba-primary-blue hover:text-uniba-primary-blue"
                                    ...
                                    aria-label="Twitter">
                                    <span class="sr-only">{{ __('contact.social.x') }}</span>
                                    <svg class="h-4 w-4" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                                        <path
                            ...
                            @if (!empty($tenant->linkedin_link))
                                <a class="inline-flex h-9 w-9 items-center justify-center rounded-full border border-uniba-border text-uniba-text-secondary transition hover:border-uniba-primary-blue hover:text-uniba-primary-blue"
                                    ...
                                    aria-label="LinkedIn">
                                    <span class="sr-only">{{ __('contact.social.linkedin') }}</span>
                                    <svg class="h-4 w-4" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                                        <path
                            ...
                            @if (empty($tenant->facebook_link) &&
                                ...
                                <span class="text-uniba-secondary">{{ __('common.placeholders.not_available') }}</span>
                            @endif
                        </div>
                    </div>
                </x-public.card>
                ...
                <x-public.card class="p-6">
                    <div class="text-sm font-semibold text-uniba-primary-blue">{{ __('contact.info_title') }}</div>
