<x-public.layout :tenant="$tenant" :title="$newsItem->title" :description="$newsItem->excerpt">
    <section class="bg-uniba-surface-0 py-20 sm:py-24">
        <x-public.container class="max-w-4xl">
            <x-public.page-header :title="$newsItem->title" :breadcrumbs="[
                ['label' => 'Home', 'href' => route('public.home')],
                ['label' => 'Berita', 'href' => route('public.news.index')],
                ['label' => $newsItem->title],
            ]">
                <x-slot:actions>
                    <x-public.button variant="outline" :href="route('public.news.index')">
            ...
            <div
                ...
                @endif
                <span>Artikel Resmi Program Studi</span>
            </div>
            ...
            @else
                <x-public.image-placeholder class="mt-6 aspect-video rounded-lg border border-uniba-border"
                    label="Gambar berita belum tersedia" iconClass="h-10 w-10" textClass="text-xs" />
            @endif
            ...
            <div class="mt-10 flex flex-wrap items-center gap-3 border-t border-uniba-border pt-6">
                <span class="text-xs font-semibold uppercase tracking-[0.12em] text-uniba-text-secondary">Bagikan</span>
                ...
                <a href="https://twitter.com/intent/tweet?url={{ urlencode(url()->current()) }}&text={{ urlencode($newsItem->title) }}"
                ...
                <a href="https://www.linkedin.com/sharing/share-offsite/?url={{ urlencode(url()->current()) }}"
                    ...
                    <span class="sr-only">Bagikan ke LinkedIn</span>
                    <svg class="h-4 w-4" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                        <path
    ...
    @if (($relatedNews ?? collect())->isNotEmpty())
        <section class="bg-uniba-surface-1 py-20">
            <x-public.container class="max-w-6xl">
                <x-public.section-heading title="Berita Terkait" subtitle="Informasi lain yang relevan untuk Anda." />
